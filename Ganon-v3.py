...
game = hlt.Game("Ganon-V3")
logging.info("Starting GanonBot-V3")
...
while True:
    ...
    for ship in my_ships:
        ...
        if not plan_executed: # NO PLAN. LET'S FIND ONE
            ...
            # Need to check all Enemies planets to see if I should still catch one (I'll accept that my enemies has one more planet than I have)

            if len(my_planets) == 0 or (len(closest_empty_planets) > 0 and bool([a for a in enemy_planets.values() if len(a) > (len(my_planets) + 1)])):
